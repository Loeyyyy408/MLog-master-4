...
def save_words_info(vector_file):  # 根据下载的预训练glove词向量，保存词向量信息,方便后续查表
    ...
    # # 创建新的 bcolz.carray 并分块写入以显示进度
    # final_vectors = bcolz.carray(np.empty((idx, vector_size)), rootdir='vectors.dat', mode='w')
    # print("分块")
    # # 将数据分块写入
    # chunk_size = 1000  # 定义块大小
    # num_chunks = (idx // chunk_size) + 1
    #
    # for i in tqdm(range(num_chunks), desc="Writing vectors"):
    #     start = i * chunk_size
    #     end = min(start + chunk_size, idx)
    #     if start < idx:
    #         # 将数据写入 final_vectors，进行 reshape
    #         final_vectors[start:end] = vectors[1:][start:end]  # 记录从第一行开始的数据
    #
    # final_vectors.flush()  # 刷新数据
...
def MutiSentence2Vector(x_in):
    ...
    for index,str in enumerate(x_in):
        ...
        try:
            ...
            means = np.zeros((300), dtype=float)
