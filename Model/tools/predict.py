    ...
    def predict_supervised(self):
        ...
        for i, (log, label) in enumerate(tbar):
            ...
            for value in log.values():
                features.append(value.clone().to(self.device))  #(32,50,768)

            if self.fer_flag:
                output,inter= self.model(features=features, device=self.device,layer_feature=True)
            else:
            ...
            #print("output:",output)
            predicted = (output < 0.5).astype(int)#BGL 0.5 HDFS:0.02 0.2
