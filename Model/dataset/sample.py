...
def trp(l, n,INT=0):#将l长度后尾补0到长度n  截断或补齐
...
def down_sample(logs, labels, sample_ratio):#按比例选定样本
    ...
    for i in tqdm(range(sample_num)):   #进度条
...
#考虑一行不足window_size情况
            #print("line",line)
            for i in range(len(line) - window_size):    #对于每个由滑动窗口生成的序列
                Sequential_pattern = list(line[i:i + window_size])#从第i个元素到第i+window_size个元素，但不包括第i+windowsize元素
                Quantitative_pattern = [0] * 2000    #创建一个全为0的列表,保存词频
                log_counter = Counter(Sequential_pattern)   #统计词频

                for key in log_counter:
                    Quantitative_pattern[key] = log_counter[key]

                Semantic_pattern = []
                Event_TF=dict()     #保存事件频率   事件index，频率
                for event in Sequential_pattern:
                    if  event == 0:  #
                        Semantic_pattern.append([0] * 300)
                    elif type(event2semantic_vec[str(event - 1)]) == type(0.1):  #type(0.1)返回float
    ...
    if sample_ratio != 1:
        result_logs, labels = (
            down_sample(result_logs, labels, sample_ratio))
...
def convert_to_tensor(string):
    # 将字符串拆分成浮点数列表
    num_list = [float(x) for x in string.split(',')]
    # 转换成 NumPy 数组
    return np.array(num_list)
def session_window(data_dir, datatype, Event_TF_IDF,template,sample_ratio=1,data_type='HDFS',soft_flag=False,fer_flag=False):
    if data_type=="HDFS":
        event2semantic_vec = read_json(data_dir + 'HDFS/events_semantic.json') #加载事件语义向量
    elif data_type=="BGL":
        event2semantic_vec = read_json(data_dir + 'BGL/events_semantic.json')
    ...
    if fer_flag:
        train_df['conv1'] = train_df['conv1'].apply(convert_to_tensor)
        train_df['conv2'] = train_df['conv2'].apply(convert_to_tensor)
        train_df['conv3'] = train_df['conv3'].apply(convert_to_tensor)
    ...
    for i in tqdm(range(len(train_df))):
        ...
        if soft_flag:
            soft_labels.append((train_df["output"][i]))
            if fer_flag:
                features['conv1'].append(train_df["conv1"][i])
                features['conv2'].append(train_df["conv2"][i])
                features['conv3'].append(train_df["conv3"][i])
    ...
    if soft_flag:
        if fer_flag:
            return result_logs, labels, soft_labels,features
        else:
            return result_logs, labels, soft_labels
    else:
        return result_logs, labels
