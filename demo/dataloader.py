...
def bgl_preprocess_data(para, raw_data, event_mapping_data):
   ...
   # print("raw_data:",raw_data)
    ...
    if not os.path.exists(sliding_file_path):
        ...
        end_index = 0
        # 设置窗口大小和步长
        window_size = pd.Timedelta(seconds=36)  # 窗口大小为36秒
        step_size = pd.Timedelta(seconds=36)  # 步长为36秒
        # get the first start, end index, end time
        for cur_time in time_data:
            cur_time= pd.to_datetime(cur_time, format='%Y-%m-%d-%H.%M.%S.%f')

            if  cur_time < start_time + window_size:
                # print(start_time + para['window_size']*3600)
                end_index += 1
                end_time = cur_time
            else:
        ...
        while end_index < log_size:
            ...
            for i in range(start_index,end_index):
                time_i=pd.to_datetime(time_data[i], format='%Y-%m-%d-%H.%M.%S.%f')
                if time_i < start_time:
                    i+=1
                else:
                    break
            for j in range(end_index, log_size):
                time_j=pd.to_datetime(time_data[j], format='%Y-%m-%d-%H.%M.%S.%f')
                if time_j < end_time:
                    j+=1
                else:
                    break
...
#    event_mapping_data = event_mapping_data.to_numpy().tolist()
    ...
    data_instance=pd.DataFrame(columns=["EventSequence","Label","Start_time","End_time"])#事件序列矩阵
    ...
    for j in tqdm(range(inst_number)):
        ...
        EventSequence=[]
        length=len(expanded_indexes_list[j])
        start=expanded_indexes_list[j][0]
        end=expanded_indexes_list[j][length-1]
        starttime=time_data[start]
        endtime=time_data[end]
        ...
        for k in expanded_indexes_list[j]:
            ...
            if label_data[k]:
                label = 1
                continue
        data_instance.loc[j]=[EventSequence,label,starttime,endtime]
        #print("EventSequence:",EventSequence)

        labels.append(label)
    #print("data_instances:",data_instance)
    data_instance.to_csv("../data/BGL_full/data_instances.csv",index=False)
...
def X_train_extract(data_dir="../data/HDFS",block_num=100000,train_ratio=0.5,normal_num=5000,normal_abnormal_ratio=1,file="../data/HDFS/data_instances.csv",template="../data/HDFS/HDFS.log_templates.csv",data_type='HDFS'):
    if data_type == 'HDFS':
        ...
        abnormal_num=int(normal_num*(1.0/normal_abnormal_ratio))  #5000
        ...
        Event_seq.to_csv("../data/"+data_type+"_full/"+data_type+".csv")
        # Event_last = Event_seq[length:]
...
def get_BGL_template(template="../data/BGL/1000000_BGL.log_templates.csv"):
 ...
 # 定义一个函数将自定义格式的 EndTime 转换为 datetime
def convert_endtime(endtime_str):
    return pd.to_datetime(endtime_str.replace('-', ' ').replace('.', ':'), format='%Y %m %d %H:%M:%S:%f')
...
if __name__=='__main__':
    ...
    """
    # para = dict()
    # para["save_path"] = "../data/BGL_full/"
    # para["step_size"] = 0.0001
    # para["window_size"] = 0.0001
    #
    # # Events_mapping指事件ID序列
    # BGL_data, Events_mapping = BGL_data(file="../data/BGL/BGL.log_structured.csv",
    #                                      template="../data/BGL/BGL.log_templates.csv")
    # # print("len", BGL_data.size)
    # # print("Events_mapping:", len(Events_mapping))
    # Events_count, labels = bgl_preprocess_data(para, BGL_data, Events_mapping)
    # print("mapping", Events_mapping)

    # X_train_extract(block_num=16852, data_type='BGL', train_ratio=0.5, file="../data/BGL/data_instances.csv", template="../data/BGL/BGL.log_templates.csv", normal_num=5000, normal_abnormal_ratio=1)



    ##事件ID映射根据 templates文件

    # templates=Detect_Events("../data/BGL_full/BGL.log_templates.csv")
    # Events_vectors(templates,events_semantic="../data/BGL_full/events_semantic.json",tf_idf=1,template="../data/BGL_full/BGL.log_templates.csv")
